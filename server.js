...
app.post("/signup", async (req,res) => {
  try {
    // deconstruct the usernmame and password from thebody
...
// login - POST - recieves a username and password, checks them agansr the database, and rea=quire the user object if they match with a cookie including a signed JWT
app.post("/login", async (req, res) => {
  try {
    // deconstruct the username and password from the body
    const { username, password } = req.body;
    // search the databse for a user with the provided username
    const user = await User.findOne({ username });
    // if no user is found, return as error
    if (!user){
      throw new Error ("No user with that username found")
    }
    // if a user is found, let's compare the provided password with the password in the user object
